...
/*   Updated: 2023/05/21 00:15:39 by hamaarou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pipe_syntax(t_parser *parser, char *cmd)
{
	...
	while (tmp->current_token->type != t_EOF)
	{
		if (tmp->current_token->type == t_PIPE)
		{
			...
			if (tmp->previous_token == NULL)
			{
				return (ff(tmp), 1);
					...
					|| type_is_rederec(tmp->previous_token) == 0)
			{
				return (ff(tmp), 1);
			}
		}
		...
		if (tmp->current_token->type == t_EOF
			&& tmp->previous_token->type == t_PIPE)
		{
			return (ff(tmp), 1);
		}
	}
...
int	redirect_syntax(t_parser *parser, char *cmd)
{
	...
	while (tmp->current_token->type != t_EOF)
	{
		...
		{
			if (tmp->previous_token
				&& (tmp->previous_token->type == t_GREAT_THAN
					|| tmp->previous_token->type == t_LESS_THAN))
				return (ff(tmp), 1);
		}
		else if (tmp->current_token->type == t_HEREDOC
				|| tmp->current_token->type == t_APPEND)
		{
			if (!tmp->previous_token
				|| tmp->previous_token->type == t_GREAT_THAN
				|| tmp->previous_token->type == t_LESS_THAN)
